...
def convert_ob_mol_to_rd_mol(ob_mol, struct=None):
    ...
    for ob_bond in ob.OBMolBondIter(ob_mol):
        ...
        elif bond_order == 12 or bond_order == 4:
...
def reconstruct_from_generated(xyz, atomic_nums, aromatic=None, basic_mode=True, bond_type=None, bond_index=None):
    """
    ...
    if bond_index is None:
        ...
        # add bonds with predicted bond types (from Pocket2Mol)
...
def reconstruct_from_generated_with_bond(xyz, atomic_nums, aromatic=None, basic_mode=True, bond_type=None, bond_index=None):
    """
    will utilize data.ligand_pos, data.ligand_element, data.ligand_atom_feature_full to reconstruct mol
    """
    if basic_mode:
...
def reconstruct_from_generated_with_edges(xyz, atomic_nums, bond_index, bond_type, raise_error=True, sanitize=True):
    # xyz = data.ligand_context_pos.clone().cpu().tolist()
    # atomic_nums = data.ligand_context_element.clone().cpu().tolist()
    # # indicators = data.ligand_context_feature_full[:, -len(ATOM_FAMILIES_ID):].clone().cpu().bool().tolist()
