...
from utils.util_data import get_dataset, ProteinElement2IndexDict, MAP_ATOM_TYPE_AROMATIC_TO_INDEX_wo_h, extract_data, \
    PreparePrefData4UI, prepare_inputs, NpEncoder, evaluation2json
...
if __name__ == '__main__':
    ...
    # log_dir = os.path.join(args.logdir, '2024')
    print('log_dir', log_dir)
    os.makedirs(log_dir, exist_ok=True)
    log_filename = os.path.join(log_dir, "log-eval.txt")
    ...
    evaluation_dir = os.path.join(f'backend/app01/static/{args.params[0]}/', 'evaluation')
    ...
    while True:
        ...
        if os.path.isfile(ckp_path):
            ...
            continue
        model_pref.load_state_dict(ckp['model_state_dict'])
        if cls_fn:
            logger.info("loading state_dict for classifier...")
            classifier.load_state_dict(ckp['cls_state_dict'])
            cls_fn = classifier
        model_pref.eval()
        calculator_pref = sampling_val(model_pref, pocket_data, pocket_idx, pos_scale, evaluation_dir, logger,
                                       ...
                                       protein_pdbqt_dir='configs/test_protein', stability_rate=0.8)
